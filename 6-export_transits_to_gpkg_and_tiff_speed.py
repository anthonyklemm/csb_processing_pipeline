# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 16:23:23 2025
...
# ---------------------------
def main():
    FILE_DIR = r'E:\csb\kuskokwim\processed\exports'
    ...
    # ------------------------------------------
    # STEP 1: Update csb table with synthetic_key and Outlier columns
    # ------------------------------------------
    try:
    ...
    # Add transit_id column if it does not exist
    try:
    ...
    # Add vessel_speed_smoothed column if needed
    try:
        con.execute("ALTER TABLE csb ADD COLUMN vessel_speed_smoothed DOUBLE;")
        print("Added vessel_speed_smoothed column to csb.")
    except Exception as e:
        print("vessel_speed_smoothed column already exists or could not be added:", e)
    ...
    for unique_id in unique_ids:
        ...
        for transit_id, group in df.groupby('transit_id'):
            ...
            print("Calculated vessel speed for transit group.")
            ...
            # --- Batch update the csb table for Outlier and transit_id ---
            updates = []
            ...
            # --- Batch update the csb table for vessel speed ---
            speed_updates = []
            for idx, row in group.iterrows():
